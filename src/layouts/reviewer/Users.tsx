    ...
    const handleSearch = (event: any) => {
        ...
        result = users.filter((data) => {
            return (
                ...
                || (data.firstName.toLocaleLowerCase() + ' ' +data.lastName.toLocaleLowerCase()).search(value) != -1 //search 'firstname lastname'
            );
        });
                    ...
                    <form className="">
                        <div className="input-group mb-2 mr-sm-2">
                            <input type="text" className="form-control"
                                id="searchUsers" placeholder="Search users" onChange={(event) =>handleSearch(event)} />
                        </div>
                    </form>
                </div>
                <div className="col-sm-12 col-md-8 text-right">
                    <BtnTwo btnType="button" label="Add new" isLink={true} link={`/create-user`} floatBottom={false} isModal={false} />
                </div>
            </div>
                        ...
                        <tbody>
                            {filteredUsers.map((user, key) => (
                                ...
                                <td className="td-preview">
                                    <Link to={{pathname: `/edit-user/${user.id}`, 
